...
* **مۆنتاژی نەرم (Pacing & Timing)**: هەمیشە هەوڵبدە لە کاتی دیالۆگدا مۆنتاژی J-Cut و L-Cut بەکاربهێنیت تا گۆڕانکاری دیمەنەکان بەشێوەیەکی خۆڕسک دەربکەون.`;
  ...
  const [onboardingStep, setOnboardingStep] = useState<1 | 2>(() => {
    const registered = localStorage.getItem("krdhub_registered") === "true";
    const activeAccount = localStorage.getItem("krdhub_active_account_id");
    if (!registered && activeAccount) {
      return 2;
    }
    return 1;
  });
